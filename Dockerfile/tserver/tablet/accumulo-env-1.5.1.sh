...
if [ -z "$HADOOP_HOME" ]
then
   test -z "$HADOOP_PREFIX"      && export HADOOP_PREFIX=/opt/hadoop/hadoop-2.4.1
else
...
test -z "$JAVA_HOME"             && export JAVA_HOME=/opt/java/jdk1.7.0_67
test -z "$ZOOKEEPER_HOME"        && export ZOOKEEPER_HOME=/opt/zookeeper/zookeeper-3.4.6
